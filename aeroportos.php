...
<div class="card" style="width: 70vw; margin-left: 15vw; margin-top: 5vh;">
  <div class="card-body">
    <h5 class="font-weight-bold mb-3">Aeroportos</h5>  
  </div> 
...
$sql = "SELECT * FROM stations WHERE email = '$login_cookie' ORDER BY `cidade` ASC";
$result = $conn->query($sql);
echo '
<thead class="black white-text">
  <tr>
    <th scope="col">Cidade</th>
    <th scope="col">Slots comprados</th>  
    <th scope="col">Slots livres</th>  
    <th scope="col">Hub</th> 
    <th scope="col"></th> 
      ...
      if ($result->num_rows > 0) {  
          while($row = $result->fetch_assoc()) {  
            echo '<tr>';  
            echo '<td>'.$row["cidade"].' ('.$row["iata"].')</td>';  
            echo '<th scope="row">'.$row["slots_empresa"].'</th>';
            echo '<th scope="row">'.$row["slots_disponivel"].'</th>';
            if($row["hub_empresa"] == 1 ){
            echo '<td>Sim</td>'; 
            }else{
            echo '<td>Não</td>'; 
            } 
            echo '<td class="d-flex justify-content-end">';    
            echo '<a href="aeroporto.php?icao='.$row["icao"].'"><button type="button" class="btn btn-teal  btn-sm m-0 px-3 md-4"><i class="fas fa-calendar-plus" aria-hidden="true"></i></button></a>'; 
            echo '</td>';
            echo '<tr>';    
          }
      } else {  
          ...
          <thead class="black white-text">
            <tr>
              <th scope="col"><center>Você ainda não tem rotas</center></th> 
            </tr>
          </thead>';
      }  
?> 
